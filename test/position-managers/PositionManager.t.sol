...
contract PositionManagerTest is Test, PosmTestSetup, LiquidityFuzzers {
    ...
    function test_modifyLiquidities_reverts_reentrancy() public {
        ...
        bytes memory calls = getMintEncoded(config, 1e18, FlagConstants.MSG_SENDER, "");
        ...
        public
    {
        ...
        mint(config, liquidityToAdd, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_mint_slippage_revertAmount0() public {
        ...
        bytes memory calls =
            getMintEncoded(config, 1e18, 1 wei, MAX_SLIPPAGE_INCREASE, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_mint_slippage_revertAmount1() public {
        ...
        bytes memory calls =
            getMintEncoded(config, 1e18, MAX_SLIPPAGE_INCREASE, 1 wei, FlagConstants.MSG_SENDER, ZERO_BYTES);
        vm.expectRevert(SlippageCheckLibrary.MaximumAmountExceeded.selector);
        lpm.modifyLiquidities(calls, _deadline);
    }
    ...
    function test_mint_slippage_exactDoesNotRevert() public {
        ...
        bytes memory calls = getMintEncoded(config, liquidity, slippage, slippage, FlagConstants.MSG_SENDER, ZERO_BYTES);
        lpm.modifyLiquidities(calls, _deadline);
        BalanceDelta delta = getLastDelta();
    ...
    function test_fuzz_burn_emptyPosition(IPoolManager.ModifyLiquidityParams memory params) public {
        ...
        uint256 tokenId;
        (tokenId, params) = addFuzzyLiquidity(lpm, FlagConstants.MSG_SENDER, key, params, SQRT_PRICE_1_1, ZERO_BYTES);
        PositionConfig memory config =
            PositionConfig({poolKey: key, tickLower: params.tickLower, tickUpper: params.tickUpper});
        assertEq(tokenId, 1);
        assertEq(lpm.ownerOf(1), address(this));
    ...
    function test_mintTransferBurn() public {
        ...
        uint256 liquidity = 100e18;
        uint256 tokenId = lpm.nextTokenId();
        mint(config, liquidity, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_decrease_increaseCurrency1_take_settle() public {
        ...
        plan.add(
            Actions.MINT_POSITION,
            abi.encode(
                configMint, 1e18, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, FlagConstants.MSG_SENDER, ZERO_BYTES
            )
        );
        plan.add(Actions.TAKE, abi.encode(key.currency0, FlagConstants.MSG_SENDER, FlagConstants.OPEN_DELTA));
        plan.add(Actions.SETTLE, abi.encode(key.currency1, FlagConstants.OPEN_DELTA, true));
        bytes memory calls = plan.finalizeModifyLiquidityWithTake(config.poolKey, FlagConstants.MSG_SENDER);
