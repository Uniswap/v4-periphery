...
contract PosMGasTest is Test, PosmTestSetup, GasSnapshot {
    ...
    function test_gas_mint() public {
        Plan memory planner = Planner.init().add(
            ...
            abi.encode(config, 10_000 ether, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, MSG_SENDER, ZERO_BYTES)
        );
    ...
    function test_gas_increaseLiquidity_erc20() public {
        ...
        mint(config, 10_000 ether, MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_multicall_initialize_mint() public {
        ...
        planner.add(
            Actions.MINT_POSITION,
            abi.encode(config, 100e18, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, MSG_SENDER, ZERO_BYTES)
        );
    ...
    function test_gas_sameRange_mint() public {
        ...
        Plan memory planner = Planner.init().add(
            Actions.MINT_POSITION,
            abi.encode(config, 10_001 ether, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, MSG_SENDER, ZERO_BYTES)
        );
        bytes memory calls = planner.finalizeModifyLiquidity(config.poolKey);
        vm.prank(alice);
        lpm.modifyLiquidities(calls, _deadline);
    ...
    function test_gas_mint_native() public {
        ...
        bytes memory calls = getMintEncoded(configNative, liquidityToAdd, MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_mint_native_excess() public {
        ...
        planner.add(
            Actions.MINT_POSITION,
            abi.encode(
                configNative, liquidityToAdd, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, MSG_SENDER, ZERO_BYTES
            )
        );
        ...
        planner.add(Actions.SWEEP, abi.encode(CurrencyLibrary.NATIVE, MSG_SENDER));
        bytes memory calls = planner.encode();
    ...
    function test_gas_increase_native() public {
        uint256 tokenId = lpm.nextTokenId();
        mintWithNative(SQRT_PRICE_1_1, configNative, 10_000 ether, MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_mint_settleWithBalance_sweep() public {
        ...
        planner.add(Actions.SWEEP, abi.encode(currency0, MSG_SENDER));
        planner.add(Actions.SWEEP, abi.encode(currency1, MSG_SENDER));
