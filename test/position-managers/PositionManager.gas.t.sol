...
contract PosMGasTest is Test, PosmTestSetup, GasSnapshot {
    ...
    function test_gas_mint_withClose() public {
        Plan memory planner = Planner.init().add(
            ...
            abi.encode(
                config, 10_000 ether, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, FlagConstants.MSG_SENDER, ZERO_BYTES
            )
        );
    ...
    function test_gas_increaseLiquidity_erc20_withClose() public {
        ...
        mint(config, 10_000 ether, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_multicall_initialize_mint() public {
        ...
        planner.add(
            Actions.MINT_POSITION,
            abi.encode(
                config, 100e18, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, FlagConstants.MSG_SENDER, ZERO_BYTES
            )
        );
    ...
    function test_gas_sameRange_mint() public {
        ...
        Plan memory planner = Planner.init().add(
            Actions.MINT_POSITION,
            abi.encode(
                config, 10_001 ether, MAX_SLIPPAGE_INCREASE, MAX_SLIPPAGE_INCREASE, FlagConstants.MSG_SENDER, ZERO_BYTES
            )
        );
        bytes memory calls = planner.finalizeModifyLiquidityWithClose(config.poolKey);
        vm.prank(alice);
    ...
    function test_gas_mint_native() public {
        ...
        bytes memory calls = getMintEncoded(configNative, liquidityToAdd, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_mint_native_excess_withClose() public {
        ...
        planner.add(Actions.SWEEP, abi.encode(CurrencyLibrary.NATIVE, FlagConstants.MSG_SENDER));
        bytes memory calls = planner.encode();
    ...
    function test_gas_increase_native() public {
        uint256 tokenId = lpm.nextTokenId();
        mintWithNative(SQRT_PRICE_1_1, configNative, 10_000 ether, FlagConstants.MSG_SENDER, ZERO_BYTES);
    ...
    function test_gas_mint_settleWithBalance_sweep() public {
        ...
        planner.add(Actions.SETTLE, abi.encode(currency0, FlagConstants.OPEN_DELTA, false));
        planner.add(Actions.SETTLE, abi.encode(currency1, FlagConstants.OPEN_DELTA, false));
        planner.add(Actions.SWEEP, abi.encode(currency0, FlagConstants.MSG_SENDER));
        planner.add(Actions.SWEEP, abi.encode(currency1, FlagConstants.MSG_SENDER));
    ...
    function test_gas_decrease_take_take() public {
        uint256 tokenId = lpm.nextTokenId();
        mint(config, 1e18, FlagConstants.MSG_SENDER, ZERO_BYTES);
        ...
        bytes memory calls = plan.finalizeModifyLiquidityWithTake(config.poolKey, FlagConstants.MSG_SENDER);
