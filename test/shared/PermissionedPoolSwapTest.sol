...
contract PermissionedPoolSwapTest is PoolTestBase {
    ...
    constructor(
        ...
        wrappedTokenFactory = IWrappedPermissionedTokenFactory(_wrappedTokenFactory);
        permissionedPositionManager = _permissionedPositionManager;
        
        // Deploy the permissioned router
        permissionedRouter = new PermissionedV4Router(
            _manager,
            permit2,
            wrappedTokenFactory,
    ...
    /// @notice Execute a swap using the permissioned router
    /// @param data The encoded swap data
    function executeSwap(bytes calldata data) external payable {
        permissionedRouter.execute{value: msg.value}(data);
    }
