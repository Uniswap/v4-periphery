...
abstract contract Notifier is INotifier {
        ...
        onlyIfApproved(msg.sender, tokenId)
    {
        ...
        bool success =
            _call(address(newSubscriber), abi.encodeWithSelector(ISubscriber.notifySubscribe.selector, tokenId, data));
    ...
    function _notifyModifyLiquidity(uint256 tokenId, int256 liquidityChange, BalanceDelta feesAccrued) internal {
        ...
        bool success = _call(
            address(_subscriber),
            abi.encodeWithSelector(ISubscriber.notifyModifyLiquidity.selector, tokenId, liquidityChange, feesAccrued)
        );
