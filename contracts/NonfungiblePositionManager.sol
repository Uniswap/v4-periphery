...
contract NonfungiblePositionManager is INonfungiblePositionManager, BaseLiquidityManagement, ERC721 {
    ...
    ///@param actions The actions encoded as (PositionActions[], bytes[]), the actions to execute and their respective parameters.
    function execute(bytes calldata actions, uint256 deadline) external payable checkDeadline(deadline) {
        execute(actions);
    }

    ///@param actions The actions encoded as (PositionActions[], bytes[]), the actions to execute and their respective parameters.
    function execute(bytes calldata actions) public external {
        // Unlock the pool manager & execute.
        poolManager.unlock(actions);
    }

    function _unlockCallback(bytes calldata actions) external returns (bytes memory) {
        (PositionActions[] memory actions, bytes[] memory params) = abi.decode(data, (PositionActions[], bytes[]));
        if (actions.length != params.length) revert LengthMismatch();

        for (uint256 i = 0; i < actions.length; i++) {
            PositionActions action = actions[i];
            bytes memory param = params[i];
            // TODO return success/or revert reason
            _execute(action, param);
        }
        _settle(); // Handles all settling against the manager. (debts through transfer, credits by mints or takes). Note that for integrating with custom hooks, we should be checking deltas applied throughout the chained execution.
    }

    function _execute(PositionActions action, bytes calldata params) internal {
        if (action == PositionActions.AddLiquidity) {
            (LiquidityRange range, uint256 liquidity, bytes calldata hookData) =
                abi.decode(params, (LiquidityRange, uint256, bytes));
            _addLiquidity(range, liquidity.toInt256(), hookData);
        }
        // TODO Support batching the following:
        // else if (action == PositionActions.Mint) {
        //     _mint(params);
        // } else if (action == PositionActions.DecreaseLiquidity) {
        //     _decreaseLiquidity(params);
        // } else if (action == PositionActions.Burn) {
        //     _burn(params);
        // } else if (action == PositionActions.Collect) {
        //     _collect(params);
        // } else {
